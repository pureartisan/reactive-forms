...
type ValidatorOrOpts<V> = PossibleValidatorFn<V> | ControlOptionsObject<V>;
...
const isOptionsObj = <V>(obj: any): obj is ControlOptionsObject<V> => {
    return Boolean(obj && !Array.isArray(obj) && typeof obj === "object");
};

const isValidator = <V, S = Validator<V> | AsyncValidator<V>>(
    obj: any
): obj is S => {
 ...
 */
export abstract class AbstractControl<V> implements BaseControl<V> {
    value?: V;
     ...
     */
    setValidators(
        newValidator: ValidatorFn<any> | ValidatorFn<any>[] | null
    ): void {
     ...
     */
    get(path: string | Array<string | number>): AbstractControl<V> | undefined {
    ...
    protected runValidator(): any | null {
        return this.validator
            ? this.validator((this as unknown) as BaseControl<V>, this.root)
            : null;
    }
    ...
    protected runAsyncValidator(emitEvent: boolean): void {
        if (this.asyncValidator) {
            ...
            const obs = Observable.toObservable(
                this.asyncValidator(
                    (this as unknown) as BaseControl<V>,
                    this.root
                )
            );
...
export class FormGroup<V extends FormGroupValue> extends AbstractControl<V> {
    group: InputGroup;
     ...
     * @return {void}
     */
    addControl<V>(name: string, control: AbstractControl<V>): void {
     ...
     * @return {void}
     */
    setControl<V>(name: string, control: AbstractControl<V>): void {
        if (this.controls[name]) {
            this.controls[name].registerOnCollectionChange();
            delete this.controls[name];
        }
...
export class FormArray extends AbstractControl<any> {
     ...
     * @return {AbstractControl}
     */
    at<V>(index: number): AbstractControl<V> | undefined {
     ...
     * @param {AbstractControl} control
     */
    insert<V>(index: number, control: AbstractControl<V>): void {
     ...
     * @param {AbstractControl} control
     */
    setControl<V>(index: number, control: AbstractControl<V>): void {
        if (this.controls[index]) {
            this.controls[index].registerOnCollectionChange();
        }
        this.controls.splice(index, 1);
